...
def link_to_list(link):
    ...
    if link and link.rest:
        l = [link.first]+link_to_list(link.rest)
    elif link:
        l = [link.first]
    return l
# Trees
...
def is_bst(t):
    ...
    f = True
    if len(t.branches)==1 and not is_bst(t.branches[0]):
        f = False
    elif len(t.branches)==2 and (t.label > bst_min(t.branches[1]) or t.label < bst_max(t.branches[0])or not is_bst(t.branches[1]) or not is_bst(t.branches[0])) :
        f = False
